...
| Q-07 | เหตุละเมิด: การตีความ “แจ้งล่าช้าไม่เกิน 15 วัน” · ช่องทางยื่นแบบแจ้ง สคส. (ระบบบันทึก `submission_ref` ของการยื่นด้วยมือ) | **ใช้งานแล้ว** (BRE-08, `LateReasonRequired`): บังคับกรอก late_reason เมื่อเกิน 72 ชม. — กรอบ 15 วันยังไม่บังคับเป็น hard limit (แค่ flag `is_late`) จนกว่าฝ่ายกฎหมายจะยืนยันการตีความ | ฝ่ายกฎหมาย |
...
| Q-21 | เมื่อเจ้าของข้อมูลที่ยินยอมไว้แล้ว (ACTIVE) ไม่ติ๊กวัตถุประสงค์นั้นในฟอร์ม ควรถือเป็นการถอนความยินยอมหรือไม่ (ST-01 ไม่มี transition NOT_CONSENTED ออกจาก ACTIVE) · ฟอร์มสาธารณะไม่ยืนยันตัวตน ใครรู้อีเมลก็ส่งแทนได้ | **ไม่เปลี่ยนสถานะ** (ไม่บันทึก transaction; ถ้าทั้งฟอร์มไม่มีอะไรเปลี่ยน = 422 `no_change`) · การถอนต้องเป็นการตัดสินใจ `WITHDRAWN` โดยตรง — ตอนนี้ผ่านเจ้าหน้าที่ ต่อไปผ่านศูนย์ตั้งค่าที่ยืนยันตัวตน (CON-18/19) · ฟอร์มสาธารณะถอนไม่ได้ | DPO / ฝ่ายกฎหมาย · CON-13, CON-18 |
| Q-22 | re-consent เมื่อเผยแพร่ข้อความฉบับ material ที่ `requires_reconsent` (CON-12): เปลี่ยนสถานะ ACTIVE → PENDING อัตโนมัติ และให้ยืนยันใหม่อย่างไร (ST-01 ยังไม่มี PENDING + CONSENTED → ACTIVE) | แสดงธง "ต้องขอใหม่" บนโปรไฟล์เท่านั้น สถานะคง ACTIVE · ยินยอมซ้ำด้วยฉบับใหม่ = `EXTENDED` | DPO · CON-12, CON-20 |
| Q-23 | บันทึกการแจ้ง สคส. (BRE-08/09): `breach.pdpc_notifications.document_version_id` เป็น NOT NULL อ้าง `platform.document_versions` ของ document composer (PLT-16) ที่ยังไม่มี — ทำ PLT-16 ก่อน หรือเพิ่ม migration ให้คอลัมน์นี้ว่างได้เพื่อบันทึกการยื่นด้วยมือ (เลขรับ + ไฟล์หลักฐาน) ไปก่อน | **เสร็จแล้ว** (BRE-08/09, บน PLT-16) — บันทึกรอบแจ้ง สคส. ผูกกับเวอร์ชันเอกสาร `pdpc_form` ที่เผยแพร่แล้ว (maker-checker, ห้ามอนุมัติงานตัวเอง), แจ้งล่าช้าบังคับกรอกเหตุผล (`LateReasonRequired`), ทางออกจากสถานะ "แจ้ง สคส." (notifying → remediating) ต้องมีรอบแจ้งที่ยืนยันแล้ว และถ้าคำตัดสินคือแจ้งทั้ง สคส.และเจ้าของข้อมูล ต้องมีการแจ้งเจ้าของข้อมูลสำเร็จอย่างน้อยหนึ่งรายด้วย | ปิดแล้ว · BRE-08, BRE-09 |
| Q-24 | ORG-02 (หลายนิติบุคคล/บริษัทในเครือ): backlog ระบุ `ขึ้นกับ` แค่ ORG-04 (เสร็จแล้ว) แต่ acceptance criterion จริง ("ผู้ใช้บริษัท A ไม่เห็นข้อมูลบริษัท B ถ้าไม่ได้รับ scope") คือการบังคับ data scope ต่อ legal entity ให้ผู้ใช้แต่ละคน ซึ่งเป็นงานของ permission engine (IAM-02, ยัง blocked อยู่ที่ Keycloak/IAM-01 — Q-18) ไม่ใช่ของ ORG. ทำ ORG-02 ตอนนี้จะพิสูจน์ได้แค่ตัวสลับ/กรองบริษัทใน UI โดยยังไม่มีการบังคับขอบเขตจริง เพราะ `iam.role_assignments.legal_entity_id` (มีในตารางแล้ว) ยังไม่มี service/UI ให้กำหนด และ AuthZ ยังไม่อ่านคอลัมน์นี้ | **เลื่อน ORG-02 ไปหลัง IAM-02** ตามที่ผู้ใช้เลือก (ไม่ทำ UI ที่ดูเหมือนบังคับ scope ได้จริงทั้งที่ยังไม่ได้บังคับ) — `org.legal_entities.parent_id` (ORG-01) และ `legal_entity_id` บนทุกตารางธุรกิจ (migration 00018) รองรับหลายบริษัทในระดับข้อมูลอยู่แล้ว รอแค่ชั้นบังคับสิทธิ์ | ผู้ใช้ / สถาปนิก · ORG-02, IAM-02 |
