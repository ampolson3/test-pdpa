...
| Q-23 | บันทึกการแจ้ง สคส. (BRE-08/09): `breach.pdpc_notifications.document_version_id` เป็น NOT NULL อ้าง `platform.document_versions` ของ document composer (PLT-16) ที่ยังไม่มี — ทำ PLT-16 ก่อน หรือเพิ่ม migration ให้คอลัมน์นี้ว่างได้เพื่อบันทึกการยื่นด้วยมือ (เลขรับ + ไฟล์หลักฐาน) ไปก่อน | **PLT-16 เสร็จแล้ว** (ผู้ใช้เลือกทำ PLT-16 ก่อน) — `breach.pdpc_notifications.document_version_id` อ้าง `platform.document_versions` ของ doc_type `pdpc_form` ได้แล้ว; BRE-08/09 (บันทึกการแจ้งบนเอกสาร) ยังไม่ได้สร้าง — notifying → remediating ยังตอบ 409 `breach.pdpc_notice_missing` จนกว่าจะทำ | ผู้ใช้ / สถาปนิก · BRE-08, BRE-09 |
